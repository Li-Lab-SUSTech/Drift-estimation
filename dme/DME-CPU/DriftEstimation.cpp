...
#include <mex.h> //add by shiwei 20210425
#include "Vector.h"
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
	...
	int* nIterations = 0;
	int nItet = 0;
	int (*progcb)(int iteration, const char* info) = NULL;


	coords_ = (float*)mxGetData(prhs[0]);
	crlb_ = (float*)mxGetData(prhs[1]);
	spotFramenum = (int*)mxGetData(prhs[2]);
	numspots = (int)mxGetScalar(prhs[3]);
	maxiterations = (int)mxGetScalar(prhs[4]);
	drift = (float*)mxGetData(prhs[5]);
	framesPerBin = (int)mxGetScalar(prhs[6]);
	gradientStep = (float)mxGetScalar(prhs[7]);
	maxdrift = (int)mxGetScalar(prhs[8]);
	scores = (float*)mxGetData(prhs[9]);
	flags = (int)mxGetScalar(prhs[10]);
	maxneighbors = (int)mxGetScalar(prhs[11]);
	nIterations = (int*)mxGetData(prhs[12]);


	if (flags & DME_3D)
	{
		mexPrintf("cpu flags = %d\n", flags);
		
		nItet = MinEntropyDriftEstimate_<3>(coords_, crlb_, spotFramenum, numspots, maxiterations, drift, framesPerBin, gradientStep, maxdrift, scores, flags, maxneighbors, progcb);
	...
	else
	{
		...
		nItet = MinEntropyDriftEstimate_<2>(coords_, crlb_, spotFramenum, numspots, maxiterations, drift, framesPerBin, gradientStep, maxdrift, scores, flags, maxneighbors, progcb);
