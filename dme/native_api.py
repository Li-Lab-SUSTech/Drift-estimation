...
import sys
import tqdm
import scipy.io #add by shiwei 2021/04/25
...
#        self._GetDeviceMemoryAllocation = smlmlib.GetDeviceMemoryAllocation
    ...
    def MinEntropyDriftEstimate(self, positions, framenum, drift, crlb, iterations, 
        ...
        scipy.io.savemat('test.mat', dict(coords=positions.T, 
                                          crlb=crlb.T,
                                          framenum=framenum,
                                          numspots=len(positions),
                                          maxit=iterations,
                                          drift=drift.T,
                                          framesperbin=framesPerBin,
                                          gradientStep=stepsize,
                                          maxdrift=maxdrift,
                                          scores=scores,
                                          flags=flags,
                                          maxneighbors=maxneighbors,
                                          progcb_mat=self.ProgressCallback(progcb),
                                          ))  
        ...
        #add by shiwei 2021/04/25
        scipy.io.savemat('result.mat', dict(drift=drift, 
                                                  scores=scores,
